...
import { EditarCamadaComponent } from '../editar-camada/editar-camada.component';
...
export class CamadaLechonesComponent {
  ...
  confirmDelete() {
    this.camadaLechonesService.delete(this.ToDelete.id).subscribe(
      () => {
  ...
  editar(contenedorLocal: any) {
    ...
    const modalRef = this.modalService.open(EditarCamadaComponent, {
      size: 'ls',
      backdrop: 'static',
      keyboard: false,
